...
export default function PassphraseGenerator({ setWallet }: Props) {
  ...
  useEffect(() => {
    ...
    return function cleanup() {
      document.removeEventListener("mousemove", seedGenerator as any, true);
    };
  });
  ...
  const seedGenerator = async (nativeEvent: MouseEvent) => {
    ...
    if (shouldTrigger && (!data || data.percentage < 100)) {
      let localData = data;
      setLastCaptured({
        x: nativeEvent.pageX,
        y: nativeEvent.pageY
      ...
      const oldSeed = localData ? localData.seed : zeroSeed;
    ...
    } else if (data && data.percentage >= 100 && !passphrase) {
  ...
  const percentage = () => {
    const number = data ? parseFloat(data.percentage + "").toFixed(1) : 0;
    ...
    <div>
      <div className="columns">
        <div className="column is-8 is-offset-2">
          ...
          {/*: */}
          <p className="">
            Move your mouse to generate random bytes, this will help making your
            wallet safer.
          </p>
        </div>
      </div>
