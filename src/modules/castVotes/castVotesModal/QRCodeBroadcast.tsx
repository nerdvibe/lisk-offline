...
export default function QRCodeBroadcast({
  ...
  const nextQRCode = () => {
    currQRCode < qrCodeValue.length - 1
      ? setCurrQRCode(currQRCode + 1)
      : setCurrQRCode(qrCodeValue.length - 1);
  };
  const prevQRCode = () => {
    ...
    <div className="modal-card">
      ...
      <section className="modal-card-body">
        <h1 className="subtitle">
          Okay now broadcast your transaction by scanning the following QR Code
          {isMultiQRCode && "s"}.
        </h1>
        {isMultiQRCode && (
          <h2 className="subtitle">{`${currQRCode + 1} of ${
            qrCodeValue.length
          } QRcodes`}</h2>
        )}
        ...
        <p className="top25">
          The votes are not valid until{" "}
          {isMultiQRCode && (
            <span>
              <strong>all</strong> the QR codes have been scanned and{" "}
            </span>
          )}{" "}
          the tx is broadcasted. To broadcast the TX scan the QR code with{" "}
          {broadcasterUrl}
        </p>
        <div />
      </section>
